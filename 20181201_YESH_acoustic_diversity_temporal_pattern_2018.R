...
setwd("C:\\Users\\Martin\\Documents\\working_folder\\ARU\\2018_analysis")
...
YESH <- fread("YESH_acousticindex_2018.csv")
...
################################################################################################################
###################### Removing MT24_above_main_colony loc not suitable: lower indices probably due to placement away from cliff edge ###############################
########################################################################################
dat<-dat %>% 
  filter(ARU %in% c("G1_isopu", "MT09_RM01", "MT09_RM04A", "MT17_cominotto", "MT17_santamarija", "MT22_stpaul", "MT24_above_north_wall", "MT24_cumnija", "MT24_delli"))
...
fullmod<-lmer(ACD50max~month+hr+prop.illuminated+moon.elevation+sun.elevation+v_wind+col.size+(1|ARU), data=dat[!is.na(dat$col.size),])
redmod<-lmer(ACD50max~month+hr+prop.illuminated+moon.elevation+sun.elevation+v_wind+(1|ARU), data=dat[!is.na(dat$col.size),])
...
##############Addition by SO on 30/11/2018 to correctly predict colony size, modified by MA to incude ACD50

RFcol<-randomForest(col.size~BIXmax+ACD50max+month+hr+prop.illuminated+moon.elevation+sun.elevation+v_wind+airtemp+dist.ARU, data=dat[!is.na(dat$col.size),],ntree=1500,mtry=6,importance=T,replace=F)

varImpPlot(RFcol)
RFcol
VAR<-importance(RFcol, nperm = 100, OOB=T)
IMP<-data.frame(variable=row.names(VAR), VAR)
IMP<-IMP[order(IMP$IncNodePurity, decreasing=T),]  ## SORTED BY node homogeneity
IMP$rel_imp<-round((IMP$IncNodePurity/IMP$IncNodePurity[1])*100,2)
IMP
...
PREDICTIONS<- dat %>%
  mutate(pred.size=predict(RFcol, newdat=dat,type="response"))%>%
  group_by(ARU) %>%
  summarise(pred.col.size=mean(pred.size), actual.col.size=mean(col.size))
names(PREDICTIONS$pred.col.size)<-NULL
PREDICTIONS<- PREDICTIONS[!(is.na(PREDICTIONS$pred.col.size)),] %>%
...
fwrite(PREDICTIONS, "predicted_colony_size.csv")

RFcol<-randomForest(col.size~ACD50max+month+hr+prop.illuminated+moon.elevation+sun.elevation+v_wind+airtemp+dist.ARU, data=dat[!is.na(dat$col.size),],ntree=1500,mtry=6,importance=T,replace=F)
