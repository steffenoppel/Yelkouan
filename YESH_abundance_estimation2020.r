...
## UPDATED ON 4 august 2020 TO INCLUDE 2020 DATA
## REQUIRES MANUAL ADJUSTMENT OF ARRAY DIMENSIONS [to ensure that additional year columns are in capture history and effort matrices]
...
## added on 4 AUG 2020 by steffen oppel based on Martin Austad's suggestion how to pool sites

## create lookup-table
RM01 <- c("MT09_RM01")
RM03 <- c("MT09_RM03", "MT09_RM03_North", "MT09_RM03_South") 
...
Cominotto <- c("MT17_Cominotto_2", "MT17_Cominotto_1", "MT17_Cominotto_3", "MT17_Cominotto_4", "MT17_Cominotto_8", "MT17_Cominotto_9") 
...
all_lut<-data.frame(orig=as.character(c(RM01,RM03,RM04,RM05,Cominotto,StPauls,Majjistral_main,Majjistral_south)),
                    poolloc=as.character(c("RM01",rep("RM03",3),rep("RM04",4),rep("RM05",8),rep("Cominotto",6),rep("StPauls",2),rep("Majjistral_main",4), rep("Majjistral_south",4))),
                    maincol=as.character(c(rep("RdumTalMadonna",16),rep("Cominotto",6),rep("StPauls",2),rep("Majjistral",8))))
...
for (col in 1:max(COLEFF$COL_NR)) {
  ...
  CHcol[1:col.n[col],,col]<-as.matrix(YESH[YESH$COL_NR==col,4:12])
  
  ## populate capture effort
  sitespercol<-max(COLEFF$SITE_NR[COLEFF$COL_NR==col])
  eff.arr[1:sitespercol,,col]<-as.matrix(COLEFF[COLEFF$COL_NR==col,3:11])
  
  ## populate period array for length of survival periods between occasions
  per.arr[1:sitespercol,,col]<-as.matrix(DURMAT[DURMAT$COL_NR==col,3:11])
...
save.image("YESH_JS_model_output2020.RData")
...
fwrite(export,"YESH_Malta_Abundance_estimates2020.csv")
...
ggplot(data=export[1:8,],aes(y=Mean, x=seq(2012.5,2019.5,1))) + geom_point(size=2)+
  geom_errorbar(aes(ymin=lcl, ymax=ucl), width=.1)+
  scale_x_continuous(name="Year", limits=c(2012,2020), breaks=seq(2012,2020,1), labels=seq(2012,2020,1))+
...
ggsave("YESH_abundance_2013_2020.pdf", device = "pdf", width=9, height=9)
