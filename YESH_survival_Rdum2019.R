...
## re-run model for only Rdum tal Madonna on 13 Nov 2019 to avoid 'polluting' dataset with data from other colonies
...
effort <- effort %>%
            ...
            dplyr::filter(COLO=="RdumTalMadonna")
...
length(unique(yesh$ringnumber))  ## 867 at RdumTalMadonna
...
## CREATE MATRIX THAT SPECIFIES HOW MUCH TIME PASSED BETWEEN OCCASIONS AT EACH SITE
...
# Bundle data
jags.data <- list(y = CH, n.years = n.years,n.ind=n.ind,f=f,
                  sitevec=sitevec, 
                  periods=periods, effmat=effmat,
                  n.sites=n.sites)
...
# Call JAGS from R
YESHsurv <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\Malta\\Analysis\\Survival_analysis\\Yelkouan\\YESH_CJS_Rdum_surv.jags",
